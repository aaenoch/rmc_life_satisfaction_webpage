...
cantril_subset <- cantril %>% 
  filter(Year %in% c("2011", "2023")) %>% 
  pivot_wider(names_from = "Year",
              values_from = `Cantril ladder score`) %>% 
  mutate(
    absolute_change = `2023` - `2011`,
    relative_change = scales::percent((`2023` - `2011`) / `2011`, accuracy = 0.01))

reactable(cantril_subset)
```
...
cantril_country <- cantril %>%
  filter(Country == c("Ghana","South Africa","Kenya", "Nigeria", "Senegal","Rwanda"))

cantril_country_plot <- ggplot(data=cantril_country,
       aes(x=Year,
  ...
  geom_line()+
  scale_y_continuous()+
  labs(title = "Cantril ladder score over years")+
  theme_minimal()

  
  
ggplotly(cantril_country_plot)
...
cantril_full <-  full_join(cantril_high,cantril_low)




happy_plot <- ggplot(data = cantril_full,
       aes(y = reorder(Country, `Cantril ladder score`),
           x = `Cantril ladder score`,
           fill = Country)) +
  geom_col()+
  labs(y="Country",
       title = "Happiness levels in 2023",
       subtitle = "The top ten happiest and least happiest countries")+
  theme_minimal()
